...
import (
	...
	"github.com/storyscript/auth/http/httpfakes"
)
...
var _ = Describe("The auth handlers", func() {
	...
	BeforeEach(func() {
		...
		userInfoFetcher.GetUserReturns(auth.User{
			Name: "test-user-name",
		}, nil)
	...
	Describe("the callback request handler", func() {
		...
		It("saves the fetched user along with their access token", func() {
			...
			Expect(userRepository.SaveArgsForCall(0)).To(Equal(auth.User{
				Name:       "test-user-name",
		...
		When("fetching a user fails", func() {
			BeforeEach(func() {
				userInfoFetcher.GetUserReturns(auth.User{}, errors.New("explode"))
			})
